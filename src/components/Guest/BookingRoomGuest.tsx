...
export default function BookingRoomGuest({ onAddSuccess }: any) {
  ...
  const handleSubmit = () => {
    form
      .validateFields()
      .then((values) => {
        values.from_time = `${selectedDate} ${moment(values.from_time, "hh:mm A").format('HH:mm:ss')}`;
        values.to_time = `${selectedDate} ${moment(values.to_time, "hh:mm A").format('HH:mm:ss')}`;
        setStep(2);
        setStep1Data(values);
      })
      .catch(errorInfo => {
      ...
      {step === 1 && (
        ...
        >
                ...
                <Form.Item label="Date*:" colon={false}>
                  <Row gutter={30} >
                    <Col className="gutter-row" >
                      {/* <Form.Item name="date" 
                    > */}
                      <DatePicker style={{ width: 181, height: 44, borderRadius: 8, textAlign: 'center' }} placeholder={today.format('DD MMM YYYY')} onChange={onChange}>
                      </DatePicker>
                      {/* </Form.Item> */}
                    </Col>
                    <Col className="gutter-row" >
                      <Input
                        style={{ width: 181, height: 44, borderRadius: 8, textAlign: 'center' }}
                      />
                    </Col>
                  </Row>
                </Form.Item>
          ...
          <div className={styles.progress}>
            <Progress percent={currentProgress} showInfo={false} strokeColor="#388697" style={{
              width: "50%",
              display: "flex",
